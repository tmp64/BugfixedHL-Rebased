...
DEFINE_HUD_ELEM(CHudSayText);

void CHudSayText::Init(void)
{
	BaseHudClass::Init();

	HookMessage<&CHudSayText::MsgFunc_SayText>("SayText");
	...
	for (int i = 0; i < MAX_LINES; i++)
	{
		if (*g_szLineBuffer[i])
		{
			if (*g_szLineBuffer[i] == 2 && g_pflNameColors[i])
			{
				...
				if (buf)
				{
					//char buf[MAX_PLAYERNAME_LENGTH+32];

					// draw the first x characters in the player color
					strncpy(buf, g_szLineBuffer[i], min(g_iNameLengths[i], MAX_PLAYERNAME_LENGTH + 32));
					buf[min(g_iNameLengths[i], MAX_PLAYERNAME_LENGTH + 31)] = 0;
