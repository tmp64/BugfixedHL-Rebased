    ...
    : CMenuPanel(iTrans, iRemoveMe, x, y, wide, tall)
{
	...
	pLabel->setText(CHudTextMessage::BufferedLocaliseTextString("#Title_SelectYourClass"));
	...
	for (int i = 0; i <= PC_RANDOM; i++)
	{
		...
		CHudTextMessage::LocaliseTextString("#Title_CurrentlyOnYourTeam", m_sPlayersOnTeamString, STRLENMAX_PLAYERSONTEAM);
	...
	m_pCancelButton = new CommandButton(CHudTextMessage::BufferedLocaliseTextString("#Menu_Cancel"), CLASSMENU_TOPLEFT_BUTTON_X, 0, CLASSMENU_BUTTON_SIZE_X, CLASSMENU_BUTTON_SIZE_Y);
