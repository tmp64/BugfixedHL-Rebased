...
void CL_DLLEXPORT HUD_ChatInputPosition(int *x, int *y)
{
	...
	{
		if (CHudSpectator::Get()->m_pip->value == INSET_OFF)
		{
			*y = YRES(PANEL_HEIGHT);
		}
		else
		{
			*y = YRES(CHudSpectator::Get()->m_OverviewData.insetWindowHeight + 5);
		}
	}
}
...
void SpectatorPanel::ActionSignal(int cmd)
{
	...
	case SPECTATOR_PANEL_CMD_NEXTPLAYER:
		CHudSpectator::Get()->FindNextPlayer(true);
		break;

	case SPECTATOR_PANEL_CMD_PREVPLAYER:
		CHudSpectator::Get()->FindNextPlayer(false);
		break;
	...
	case SPECTATOR_PANEL_CMD_TOGGLE_INSET:
		CHudSpectator::Get()->SetModes(-1,
		    CHudSpectator::Get()->ToggleInset(false));
		break;
...
void SpectatorPanel::Update()
{
	...
	if (m_insetVisible)
		offset = CHudSpectator::Get()->m_OverviewData.insetWindowX + CHudSpectator::Get()->m_OverviewData.insetWindowWidth + 2;
	else
		offset = 0;

	bool visible = CHudSpectator::Get()->m_drawstatus->value != 0;
