...
public:
	...
	virtual void actionPerformed(Panel *panel)
	{
		CHudSpectator::Get()->FindPlayer(m_szplayer);
		gViewPort->HideCommandMenu();
	}
};
...
public:
	...
	void paint()
	{
		...
		if (CHudHealth::Get()->m_fFade)
		{
			CHudHealth::Get()->m_fFade -= (gHUD.m_flTimeDelta * 20);
			if (CHudHealth::Get()->m_fFade <= 0)
			{
				a = MIN_ALPHA;
				CHudHealth::Get()->m_fFade = 0;
			}

			// Fade the health number back to dim
			a = MIN_ALPHA + (CHudHealth::Get()->m_fFade / FADE_TIME) * 128;
		}
		else
			a = MIN_ALPHA;

		CHudHealth::Get()->GetPainColor(r, g, b);
		ScaleColors(r, g, b, a);

		// If health is getting low, make it bright red
		if (CHudHealth::Get()->m_iHealth <= 15)
			a = 255;
		...
		int x = gHUD.DrawHudNumber(iXPos + iXSize + 5, iYPos + 5, DHN_3DIGITS | DHN_DRAWZERO, CHudHealth::Get()->m_iHealth, r, g, b);
