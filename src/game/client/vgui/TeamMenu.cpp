    ...
    : CMenuPanel(iTrans, iRemoveMe, x, y, wide, tall)
{
	...
	pLabel->setText("%s", CHudTextMessage::BufferedLocaliseTextString("#Title_SelectYourTeam"));
	...
	m_pBriefing->setText(CHudTextMessage::BufferedLocaliseTextString("#Map_Description_not_available"));
	...
	for (int i = 1; i <= 5; i++)
	{
		...
		if (i == 5)
		{
			...
			m_pButtons[5]->setText(CHudTextMessage::BufferedLocaliseTextString("#Team_AutoAssign"));
	...
	for (int i = 1; i <= 4; i++)
	{
		if (m_pButtons[i])
		{
			if (i <= gViewPort->GetNumberOfTeams())
			{
				...
				char szPlayerList[(MAX_PLAYERNAME_LENGTH + 3) * 31]; // name + ", "
				strcpy(szPlayerList, "\n");
				...
				if (iTotal > 0)
				{
					...
					char szText[((MAX_PLAYERNAME_LENGTH + 3) * 31) + 256];
					if (iTotal == 1)
